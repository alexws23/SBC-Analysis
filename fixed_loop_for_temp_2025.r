...
data_files <- list.files(pattern = "_2\\.csv$", path = "./2025_outputs/")
...
for (file in data_files) {
  ...
  file_name <- paste0("2025_FINAL/", county_name, "_FINAL.csv")
  
  # Read in the data
  data <- read.csv(paste0("./2025_outputs/",file), header = TRUE, sep = ",")
  ...
  colnames(data_4) <- c("date", "precip", "rain", "snowfall", "tmax", "tmin", "latitude", "longitude", "location_id", "year",
  ...
  data_5 <- bind_rows(all_results) %>%
    pivot_wider(
      ...
      values_from = c(GGD_1, GGD_2, avg_tmin, avg_tmax, total_precip,total_rain,total_snow),
...
write.csv(final_combined, "AllCounties_FINAL_2025.csv", row.names = FALSE)
